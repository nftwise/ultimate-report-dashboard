...
export default function DateRangePicker({ dateRange, onDateRangeChange }: DateRangePickerProps) {
  ...
  // Sync calMonth to current range when opening
  const handleOpen = () => {
  ...
  const handleDayClick = (day: Date) => {
    if (day > yesterday) return; // Block future dates

    if (!startDate) {
      // First click → set start
      setStartDate(day);
    } else {
      // Second click → confirm range (auto-swap if needed)
  ...
  // Compute visual range for highlighting
  const visualFrom = startDate && hoverDate
    ? (hoverDate < startDate ? hoverDate : startDate)
    : null;
  const visualTo = startDate && hoverDate
    ? (hoverDate < startDate ? startDate : hoverDate)
    : null;

  const isInRange = (day: Date) => {
    if (!visualFrom || !visualTo) return false;
    return day > visualFrom && day < visualTo;
  };
  ...
  return (
    <div ref={wrapperRef} style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
      ...
      {open && dropdownPos && (
        <div style={{
          ...
          {/* Cancel — only show when selecting */}
          {startDate && (
            <button
              onClick={() => { resetTemp(); }}
              style={{
